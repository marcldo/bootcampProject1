...
function showcurrentlocationdata(lat, long) {
  //For TicketMaster
  var tickemquery = `https://app.ticketmaster.com/discovery/v2/events?apikey=7elxdku9GGG5k8j0Xm8KWdANDgecHMV0&latlong=${lat},${long}`;
  console.log(tickemquery);
  fetch(tickemquery)
    .then(resp => resp.json())
    .then(response => {
      console.log(response);
      ...
      console.log(includesCityorState(response));
      if (checkEvent === 0) {
        $("#error").text(`Sorry Events Found`);
      } else var noEventTable = $("<table>");
      noEventTable = `<tr><th> EVENTS </th></tr> <tr><td>No events at this location</td></tr>`;
      $("#eventsData").append(noEventTable);
      $(".modal").modal("hide");
  ...
  var zQueryURL = `https://developers.zomato.com/api/v2.1/collections?lat=${lat}&lon=${long}`;
  ...
  $.ajax({
    ...
    //fetch(zQueryURL).then(resp=>resp).then(response=>{
  }).then(function(response) {
    // checking if collections property exists in JSON response
    console.log(response);
    //call zomatodata function to populate the div with zomato data
